...
resource "null_resource" "n1" {
  ...
  provisioner "remote-exec" {
    inline = [
        "sudo useradd ndayishimiye",
        "mkdir terraform1",
    ]
  }
  provisioner "file" {
    source = "week12key.pem"  # path where you copy to the local file or directory
    destination = "/tmp/key.pem" # path to be pasted in the server
  }
